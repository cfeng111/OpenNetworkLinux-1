...
port_to_cpld_present_offset(int port)
{
    ...
    if (port < QSFP_START_INDEX)
    {
        ...
        index = port / 8;
        ret = index + PORT_CPLD_SFP_PRESENT_ADDR_OFFSET; //Present of Port 1~8
    }
    else
    {
...
onlp_sfpi_control_get(int port, onlp_sfp_control_t control, int *value)
{
    ...
    int cpld_addr = port_to_cpld_addr(port);
    int cpld_offset = 0;
    int port_bit = port_to_cpld_port_bit(port);
    int supported = 0;
    char optval = 0;
    ...
    *value = 0;
    ...
    if (port < QSFP_START_INDEX) //SFP
    {
        ...
        {
            case ONLP_SFP_CONTROL_RX_LOS:
                cpld_offset = port_to_cpld_sfp_offset(port, PORT_CPLD_SFP_RX_LOS_ADDR_OFFSET);
                break;
            case ONLP_SFP_CONTROL_TX_FAULT:
                cpld_offset = port_to_cpld_sfp_offset(port, PORT_CPLD_SFP_TX_FAULT_ADDR_OFFSET);
                break;
            case ONLP_SFP_CONTROL_TX_DISABLE:
                cpld_offset = port_to_cpld_sfp_offset(port, PORT_CPLD_SFP_TX_DISABLE_ADDR_OFFSET);
                break;
            default:
                return ONLP_STATUS_E_UNSUPPORTED;
                break;
        }
        optval = onlp_i2c_readb(cpld_bus, cpld_addr, cpld_offset, ONLP_I2C_F_FORCE);
        ...
        DIAG_PRINT("%s, port:%d, port_bit:%d, cpld_bus:%d, cpld_addr:%d, cpld_offset:%d", __FUNCTION__, port, port_bit, cpld_bus, cpld_addr, cpld_offset);
    }
    else //QSFP
    {
        switch (control)
        {
            case ONLP_SFP_CONTROL_RESET_STATE:
                optval = onlp_i2c_readb(cpld_bus, cpld_addr, PORT_CPLD_RESET_ADDR_OFFSET, ONLP_I2C_F_FORCE);
                if ((optval & (1 << port_bit)) != 0) //1
                {
                    *value = 1;
                }
